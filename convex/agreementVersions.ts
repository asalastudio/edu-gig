...
export const share = authedMutation({
 ...
 handler: async (ctx, args): Promise<{ contractId: Id<"contracts">; versionId: Id<"agreementVersions">; revision: number }> => {
  ...
  await ctx.db.patch(c._id, { currentSharedVersionId: version._id, revision, updatedAt: Date.now() });
...
export const listForEngagement = authedQuery({ args: { engagementId: v.id("engagements") }, handler: async (ctx, args) => {
 ...
 for (const c of contracts) {
  ...
  for (const x of all) {
   ...
   versions.push({ versionId: x._id, privateFileId: x.privateFileId, number: x.number, kind: x.kind, parentVersionId: x.parentVersionId, fileName: x.fileName, uploadedByUserId: x.uploadedByUserId, uploaderName: uploader ? `${uploader.firstName} ${uploader.lastName}`.trim() : "Unknown", createdAt: x.createdAt, sharedAt: x.sharedAt, legacyStatus: x.legacyStatus, downloadUrl: downloadPath(x.privateFileId), isMine: mine });
  }
  if (c.managed && !versions.length && !await sameParty(ctx, e, c.uploadedByUserId)) continue;
  const events = [];
  for (const event of await ctx.db.query("contractEvents").withIndex("by_contract", q => q.eq("contractId", c._id)).collect()) {
  ...
  result.push({ contractId: c._id, title: c.title, notes: c.notes, revision: c.revision ?? 0, status: c.status, legacy: !c.managed, currentSharedVersionId: c.currentSharedVersionId, versions: versions.sort((a,b) => b.number-a.number), activity: events, allowedActions: { upload: actionable && !!c.managed, share: actionable && versions.some(x => x.isMine && x.sharedAt === undefined), signedCopy: actionable && versions.some(x => x.sharedAt !== undefined), recordSigning: actionable && !!c.currentSharedVersionId } });
 }
 return result;
} });
