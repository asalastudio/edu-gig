...
| RFP→payment bridge missing | `convex/proposals.ts` (`accept`) | area/payments, type/feature, severity/blocker | ♻️ **Superseded — August 2026.** Accepting a proposal now creates an engagement. Payment is intentionally off-platform; Contract Hub stores documents. Do not build checkout from this row. |
