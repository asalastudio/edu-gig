...
it('preserves profile opening, About/Credentials/Resume, exact messaging target and both rate units',()=>{
 render(<Profile/>);expect(screen.getByText('A real professional biography')).toBeInTheDocument();expect(screen.getByRole('tab',{name:/Credentials/})).toBeInTheDocument();expect(screen.getByRole('tab',{name:/Resume/})).toBeInTheDocument();expect(screen.getAllByText('$95/hr · $650/day').length).toBeGreaterThan(0);
...
it('saves only to the current district and returns to the exact filtered directory',()=>{
 const view=render(<Profile/>);fireEvent.click(screen.getAllByRole('button',{name:/Save to List/})[0]);expect(readSavedConsultants('district-a')).toEqual(['consultant-id']);
 s.viewer={_id:'district-b',role:'district_admin'};view.rerender(<Profile/>);expect(screen.getAllByRole('button',{name:/Save to List/}).length).toBeGreaterThan(0);
